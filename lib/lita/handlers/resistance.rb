    ...
    class Resistance < Handler
      ...
      def play(response)
        ...
        spies.each do |member|
          ...
          robot.send_message(Source.new(user: user), '@' + response.user + ' has started game ID#' + gameId + 'of Resistance. You are a member of the spies.')
        end

        resistance.each do |member|
          user = Lita::User.find_by_mention_name(member)
          robot.send_message(Source.new(user: user), '@' + response.user + ' has started game ID#' + gameId + 'of Resistance. You are a member of the resistance.')
        end

        response.reply('Roles have been assigned to the selected people! This is game #' + gameId)
      end
